...
public class SimplePerceptionModule implements PerceptionModule<SimplePerceptionConfiguration> {
    ...
    public List<VehicleObject> getPerceivedVehicles() {
        ...
        List<SpatialObject> initiallyPerceivedVehicles =
                new ArrayList<>(SimulationKernel.SimulationKernel.getCentralPerceptionComponentComponent()
                        .getSpatialIndex()
                        .getVehiclesInRange(perceptionModel)
                );
        initiallyPerceivedVehicles = perceptionModel.applyPerceptionModifiers(owner, initiallyPerceivedVehicles);
        return initiallyPerceivedVehicles.stream().map(spatialObject -> (VehicleObject) spatialObject).collect(Collectors.toList());
    }
    ...
    public List<SpatialObject> getPerceivedObjects() {
        ...
        perceptionModel.updateOrigin(owner.getVehicleData().getProjectedPosition(), owner.getVehicleData().getHeading());
        SimulationKernel.SimulationKernel.getCentralPerceptionComponentComponent().updateSpatialIndices();
        List<SpatialObject> perceivedObjects = new ArrayList<>();
        perceivedObjects.addAll(SimulationKernel.SimulationKernel.getCentralPerceptionComponentComponent()
                .getSpatialIndex()
                .getVehiclesInRange(perceptionModel));
        perceivedObjects.addAll(SimulationKernel.SimulationKernel.getCentralPerceptionComponentComponent()
                .getSpatialIndex()
                .getTrafficLightsInRange(perceptionModel));
        return perceptionModel.applyPerceptionModifiers(owner, perceivedObjects);
    }
    ...
    private static class SimplePerceptionModel implements PerceptionModel {
        ...
        public List<SpatialObject> applyPerceptionModifiers(PerceptionModuleOwner owner, List<SpatialObject> perceivedVehicles) {
            List<SpatialObject> filteredList = perceivedVehicles;
