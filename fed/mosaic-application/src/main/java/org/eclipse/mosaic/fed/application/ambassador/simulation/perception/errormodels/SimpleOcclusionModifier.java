...
public class SimpleOcclusionModifier implements PerceptionModifier {
    ...
    public List<SpatialObject> apply(PerceptionModuleOwner owner, List<SpatialObject> spatialObjects) {
        ...
        double furthestPerceivedDistance = owner.getPerceptionModule().getConfiguration().getViewingRange();
        ...
        List<SpatialObject> nonOccludedObjects = new ArrayList<>();
        List<Vector3d> nonOccludedVectors = new ArrayList<>();
        nonOccludedObjects.add(sortedByDistance.get(0)); // closest vehicle is always perceived
        ...
        for (int i = 1; i < sortedByDistance.size(); i++) { // iterate over all other sorted vehicles
            ...
            isOccluded:
            {
                ...
                nonOccludedObjects.add(sortedByDistance.get(i));
                nonOccludedVectors.add(currentRelativeVector);
            }
        }
        spatialObjects.retainAll(nonOccludedObjects); // apply changes on initial list
        return spatialObjects;
    }
    ...
    /**
     * There can't be any other objects closer to the current vehicle,
